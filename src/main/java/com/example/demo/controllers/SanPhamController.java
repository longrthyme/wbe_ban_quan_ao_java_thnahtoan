...
import com.example.demo.service.SanPhamCTService;
...
//                        return ResponseEntity.ok(12);
    ...
    public ResponseEntity<Integer> createSanPhamCT(@RequestBody SanPhamCTDTO request) {
        try {
            SanPhamChiTiet savedSPCT= sanPhamCTService.createSanPhamCT(request);
            return ResponseEntity.ok(savedSPCT.getId_san_pham_ct());
        } catch (Exception e) {
    ...
    public String detail(@PathVariable("id") Integer id, Model model) {
        ...
        List<SanPhamChiTiet> sanPhamCTList = sanPhamCTRepository.findBySanPhamId(id);

        List<SanPhamChiTietDTO> spDTO = sanPhamCTRepository.findSanPhamChiTietWithThuongHieuBySanPhamId(id);

        // Add both SanPham and related SanPhamCT list to the model
        model.addAttribute("detail", sp);
        model.addAttribute("sanPhamCTList", spDTO);
    ...
    public String showUpdateForm(@PathVariable("id") Integer id, Model model) {
        SanPhamChiTiet sanPhamCT = sanPhamCTRepository.findById(id)
...
//        model.addAttribute("size", listSize);
    ...
    public String updateSanPhamCT(@PathVariable("id") Integer id,
                                  @ModelAttribute SanPhamChiTiet updatedSanPhamCT) {

        // Fetch the existing SanPhamCT based on ID
        SanPhamChiTiet existingSanPhamCT = sanPhamCTRepository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Invalid SanPhamCT ID: " + id));

        // Update the fields of the existing entity
        existingSanPhamCT.setMaSpct(updatedSanPhamCT.getMaSpct());
//        existingSanPhamCT.setId_san_pham_ct(updatedSanPhamCT.getId_san_pham_ct());
        existingSanPhamCT.setMauSac(updatedSanPhamCT.getMauSac());
        existingSanPhamCT.setIdThuongHieu(updatedSanPhamCT.getIdThuongHieu());
        existingSanPhamCT.setSize(updatedSanPhamCT.getSize());  // Map size
        existingSanPhamCT.setGia_ban(updatedSanPhamCT.getGia_ban());
        existingSanPhamCT.setSo_luong_ton(updatedSanPhamCT.getSo_luong_ton());
        existingSanPhamCT.setTrang_thai(updatedSanPhamCT.getTrang_thai());
