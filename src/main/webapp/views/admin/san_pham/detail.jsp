...
<%--                              <th scope="col">Màu sắc</th>--%>
                ...
                <c:forEach var="spct" items="${sanPhamCTList}" varStatus="status">
                          <tr id="row-${status.index+1}">
                               ...
                               <td>${spct.maSpct}</td>
<%--                               <td>${spct.mauSac.idMauSac}</td>--%>
                               <td>${spct.tenThuongHieu}</td>
                               <td>${spct.size}</td>
                               <td><fmt:formatNumber value="${spct.giaBan}" type="currency" currencySymbol="₫" /></td>
                               <td>${spct.soLuongTon}</td>
                              <td>
                                  <c:choose>
                                      <c:when test="${spct.trangThai == 1}">Còn hàng</c:when>
                                      <c:otherwise>Hết hàng</c:otherwise>
                                  </c:choose>
                              </td>

                              <td>
                                  <a href="/san-pham/update/detail/${spct.idSanPhamCT}" data-bs-toggle="tooltip" title="Cập nhật">
